		...
		*/
		...
		saveAndExit.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if(DiagnosticsUtils.calledFromCollect) {
				...
				else if(!DiagnosticsUtils.calledFromCollect) {
					instance_name = "instance_" + time + "/";
				}
				...
				try {
					...
					String sdHomeDir = DiagnosticsUtils.internalSDCardPath + "Archive/";
					...
					if(DiagnosticsUtils.calledFromCollect) {
						...
						fis = new FileInputStream(pDataPath);
						fos = new FileOutputStream(instancePath + instance_name + new File(pDataPath).getName());
						buf = new byte[1024];
						while((len = fis.read(buf)) > 0) {
							fos.write(buf, 0, len);
						}

						// copy over image file to instance dir for sending along to aggregate
						imagePath = DiagnosticsUtils.getMarkedupPhotoPath(photoName);
